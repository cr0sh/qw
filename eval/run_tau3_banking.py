...
This launcher intentionally requires ``--run``. Without it, configuration is
validated and no dataset, server, or model request is touched.
"""
...
def main() -> int:
    ...
    # EvalScope's OpenAI-compatible backend reads this fallback when api_key=None.
    # Keep the token process-local and never include it in capture or output.
    os.environ["EVALSCOPE_API_KEY"] = simulator_key

    setup_dir = Path(__file__).resolve().parent
    run_id = datetime.now(timezone.utc).strftime("%Y%m%dT%H%M%SZ") + "-" + uuid4().hex
    work_dir = setup_dir / f"tau3-banking-{run_id}"
    ...
    if Path(dataset_id).is_dir():
        # tau2 snapshots DATA_DIR at import; make the local diagnostic snapshot
        # visible before installing the bridge.
    ...
    config = TaskConfig(
        ...
        # Exceptions are skipped by EvalScope and remain visible in the adapter's
        # capture stream; successful/legitimate Tau2 rewards are unchanged.
        ignore_errors=True,
    )
