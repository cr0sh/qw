...
use support::{
    DECODE_MAX_TOKENS, LONG_CONTEXT_64K_MIN_TOKENS, load_provider, prepare_decode_fixture,
        ...
        group.bench_function(name, |bencher| {
            bencher.iter_custom(|iterations| {
                ...
                for _ in 0..iterations {
                    let (output, elapsed, stats) = provider
...
fn benchmark_64k_cached_decode(criterion: &mut Criterion) {
    ...
    let mut provider = load_provider();
    let fixture = prepare_long_conversation_fixture(&mut provider, LONG_CONTEXT_64K_MIN_TOKENS);
...
criterion_group!(benches, benchmark_fresh_decode, benchmark_64k_cached_decode);
