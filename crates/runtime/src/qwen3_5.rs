...
// Qwen3.5 Model.
fn mtp_target_cache_mode(_has_mtp: bool, requested: KVCacheMode) -> KVCacheMode {
    requested
}
...
impl Qwen35Model {
    ...
    pub(crate) fn forward_dflash_continuation(
        ...
        self.forward_dflash_prefill_segment(input_ids, target_layer_ids, hidden_limit, false, false)
    }

    #[cfg(any(feature = "dflash2", test))]
    pub(crate) fn forward_dflash_prefill_segment(
        &self,
        input_ids: &MlxArray,
        target_layer_ids: &[usize],
        hidden_limit: usize,
        ...
        while start < prompt_len {
            ...
            let final_hidden = self.sequence_state.with_internal(|caches| {
                    ...
                    self.layers.iter().zip(caches.iter_mut()).enumerate()
                {
                    ...
                    {
                        // Trim before copying so a short retained window does
                        // not keep the whole chunk's allocation alive.
                        let h_shape = mlxcel_core::array_shape(&h);
                        let keep = (h_shape[1] as usize).min(hidden_limit) as i32;
                        let h_rows = if keep < h_shape[1] {
                            mlxcel_core::copy(&mlxcel_core::slice(
                                &h,
                                &[0, h_shape[1] - keep, 0],
                        ...
                        let combined = match current {
                            Some(prior) if (keep as usize) < hidden_limit => {
                                let prior_shape = mlxcel_core::array_shape(&prior);
                                let prior_keep =
                                    (hidden_limit - keep as usize).min(prior_shape[1] as usize);
                                let prior = mlxcel_core::slice(
                                    &prior,
                                    &[0, prior_shape[1] - prior_keep as i32, 0],
                                    &prior_shape,
                                );
                                mlxcel_core::concatenate(&prior, &h_rows, 1)
                            }
                            _ => h_rows,
                        };
            ...
            unsafe { mlxcel_core::detach_all(&roots) };
            start = end;
        }
        if finish {
    ...
    pub(crate) fn load_specprefill_draft(model_dir: &Path) -> Result<Self> {
        ...
        let quantization = config.quantization.as_ref();
            ...
            config.model_type == "qwen3_5_text"
                ...
                && quantization
                    .is_some_and(|q| { q.mode == "affine" && q.bits == 8 && q.group_size == 64 }),
    ...
    #[cfg(any(feature = "specprefill", test))]
    pub(crate) fn specprefill_draft_prefill(
        &self,
        prompt_ids: &[i32],
    ) -> Result<UniquePtr<MlxArray>> {
        ensure!(
            !prompt_ids.is_empty(),
            "SpecPrefill draft prompt must not be empty"
        );
        self.reset_runtime_state();
        let configured = mlxcel_core::generate::prefill_chunk_len();
        let chunk_len =
            mlxcel_core::generate::effective_prefill_chunk(configured, true, prompt_ids.len())
    ...
    pub(crate) fn specprefill_draft_lookahead(
        ...
        self.sequence_state.with_internal(|caches| {
            ...
            let mut queries =
                Vec::with_capacity(self.layers.len() / self.config.full_attention_interval);
            for (layer, cache) in self.layers.iter().zip(caches.iter_mut()) {
    ...
    pub(crate) fn specprefill_decode(
        ...
        let position_ids = mlxcel_core::from_slice_i32(&[position, position, position], &[3, 1, 1]);
        let hidden = self.sequence_state.with_internal(|caches| {
            self.forward_backbone_with_inputs(&input, None, caches, Some(&position_ids))
        });
...
const QWEN35_TURBO4_SNAPSHOT_SUFFIXES: [&str; 5] =
...
fn attention_snapshot_mode(
    snapshot: &ModelStateSnapshot,
    index: usize,
) -> Result<KVCacheMode, String> {
    let present = |suffix: &str| {
        let name = format!("layer.{index}.{suffix}");
        snapshot.tensor(&name).is_some() || snapshot.paged_tensor(&name).is_some()
    };
    let fp16 = ["keys", "values"];
    let has_fp16 = fp16.iter().any(|suffix| present(suffix));
    let has_packed = QWEN35_TURBO4_SNAPSHOT_SUFFIXES
        .iter()
        .any(|suffix| present(suffix));
    match (has_fp16, has_packed) {
        (true, false) if fp16.iter().all(|suffix| present(suffix)) => Ok(KVCacheMode::Fp16),
        (false, true)
            if QWEN35_TURBO4_SNAPSHOT_SUFFIXES
                .iter()
                .all(|suffix| present(suffix)) =>
        {
            Ok(KVCacheMode::Turbo4)
        }
        _ => Err(format!(
            "Qwen3.5 snapshot layer {index} has mixed or incomplete attention storage"
        )),
    }
}
...
fn validate_snapshot_tensor_names(
    ...
    for (index, layer) in layers.iter().enumerate() {
        ...
        } else if attention_snapshot_mode(snapshot, index)? == KVCacheMode::Turbo4 {
            if kv_cache_mode != KVCacheMode::Turbo4 {
                return Err("Packed snapshot requires a Turbo4 model".to_string());
            }
            for suffix in QWEN35_TURBO4_SNAPSHOT_SUFFIXES {
        ...
        for (index, layer) in self.layers.iter().enumerate() {
            ...
            if layer.is_linear {
                ...
                let tensor = |suffix: &str| {
                    ...
                    if shape.len() != 4
                        || shape[0] != 1
                        || shape[1] != self.config.num_key_value_heads as i32
                        || shape[2] != token_len
...
mod tests {
    ...
    #[test]
    fn bundled_mtp_preserves_requested_target_cache() {
        assert_eq!(
    ...
    #[test]
    fn attention_snapshot_rejects_mixed_and_incomplete_storage() {
        let mut snapshot = ModelStateSnapshot::new("test", 2);
        assert!(attention_snapshot_mode(&snapshot, 0).is_err());
        let tensor = test_attention_tensor(2, 0.01);
        ...
        let mut packed = ModelStateSnapshot::new("test", 2);
        for suffix in &QWEN35_TURBO4_SNAPSHOT_SUFFIXES[..4] {
            packed.push_tensor(format!("layer.0.{suffix}"), &tensor);
        }
    ...
    fn turbo4_snapshot_uses_k_rescale_sidecar() {
        assert_eq!(
            QWEN35_TURBO4_SNAPSHOT_SUFFIXES,
            ["k_packed", "k_rescale", "v_packed", "v_norms", "v_rescale"]
        );
    }
    ...
    fn dflash_prefill_test_model() -> Qwen35Model {
        ...
        .expect("small hybrid config");
        let mut weights = WeightMap::new();
        let mut insert = |name: String, shape: &[i32], constant: Option<f32>| {
            let len = shape.iter().map(|&dim| dim as usize).product();
            let values = (0..len)
                .map(|i| constant.unwrap_or_else(|| ((i * 13 + name.len()) as f32).sin() * 0.025))
                .collect::<Vec<_>>();
            weights.insert(name, mlxcel_core::from_slice_f32(&values, shape));
        };
        insert("model.embed_tokens.weight".into(), &[32, 64], None);
        insert("model.norm.weight".into(), &[64], Some(1.0));
        for layer in 0..2 {
            let prefix = format!("model.layers.{layer}");
            for norm in ["input_layernorm", "post_attention_layernorm"] {
                insert(format!("{prefix}.{norm}.weight"), &[64], Some(1.0));
            }
            for (projection, shape) in [
                ...
                insert(format!("{prefix}.mlp.{projection}.weight"), &shape, None);
            }
            if layer == 0 {
                for (projection, shape) in [
                    ("in_proj_qkv", [96, 64]),
        ...
        Qwen35Model::from_weights(&weights, &config, KVCacheMode::Fp16)
            .expect("small hybrid weights")
    }
    ...
    fn dflash_prefill_chunks_match_fresh_resumed_and_restored_continuations() {
        ...
        let input = mlxcel_core::from_slice_i32(&ids, &[1, 29]);
        let next = mlxcel_core::from_slice_i32(&[5, 19], &[1, 2]);
        ...
        for hidden_limit in [5, usize::MAX] {
            for resume_mode in 0..3 {
                ...
                model.sequence_state.with_internal(|caches| {
                    assert!(caches.iter().all(|cache| cache.offset() == 29));
                });
                let actual_next = model.forward_dflash_verify(&next, &[0, 1]);
                assert_dflash_prefill_close(&actual_next.logits, &continuation.logits);
            }
        }
    }

    #[test]
    #[ignore = "requires an isolated GPU process: resets global MLX peak-memory counters"]
    fn dflash_prefill_chunk_peak_tracks_explicit_materialized_segments() {
        let model = dflash_prefill_test_model();
        ...
        // growth with a known bounded graph, independent of the internal loop.
        for explicit_segments in [true, false] {
            model.reset_runtime_state();
