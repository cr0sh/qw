...
pub struct ExpandedImageTokens {
    ...
    /// Expanded model-token spans, in the same order as the input image grids.
...
pub fn insert_qwen_vl_image_tokens(
    ...
    let mut image_token_ranges = Vec::with_capacity(grid_thw.len());
    let mut image_index = 0;
    for &token in prompt_tokens.iter() {
        if token == image_token_id {
            let start = expanded.len();
            expanded.extend(std::iter::repeat_n(image_token_id, counts[image_index]));
            image_token_ranges.push(start..expanded.len());
            image_index += 1;
        } else {
            expanded.push(token);
        }
...
mod tests {
    ...
    fn expands_one_placeholder_to_the_merged_grid_count() {
        ...
        assert_eq!(stats.image_token_ranges, vec![2..6]);
    }
    ...
    fn expands_multiple_images_in_declared_order() {
        ...
        assert_eq!(stats.image_token_ranges, vec![1..5, 8..10]);
    }
