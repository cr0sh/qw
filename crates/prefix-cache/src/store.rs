...
impl PersistentSnapshotStore for FilesystemSnapshotStore {
    ...
    fn load(&mut self, k: &EntryKey) -> Result<Option<StoredEntry>, String> {
        ...
        if crate::codec::entry_key(ns, m.route, crate::PromptKey::new(&m.token_ids, &m.images))
            != *k
        {
    ...
    fn load_bounded(&mut self, key: &EntryKey, limit: u64) -> Result<Option<StoredEntry>, String> {
        ...
        if crate::codec::entry_key(
            ns,
            parsed.route,
            crate::PromptKey::new(&parsed.token_ids, &parsed.images),
        ) != *key
        {
            return Err("cache entry digest mismatch".into());
        }
