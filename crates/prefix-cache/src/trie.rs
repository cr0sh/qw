...
impl RadixTrie {
    pub fn new() -> Self {
        Self {
            nodes: vec![Some(Node { fragment: Vec::new(), parent: None, children: Vec::new(), terminals: Vec::new() })],
        }
    }

    pub fn observe(&mut self, tokens: &[i32], route: SnapshotRoute, now: u64, expires_at: u64) -> usize {
        ...
        loop {
            ...
            if consumed == tokens.len() {
                if self.terminal(node_id, route).is_none() {
                    self.node_mut(node_id).terminals.push(Terminal::structural(route, 1, now, expires_at));
                }
                return node_id;
            }
            let next = tokens[consumed];
            let child = self.node(node_id).children.iter().copied().find(|child| self.node(*child).fragment[0] == next);
            ...
            let common = fragment.iter().zip(&tokens[consumed..]).take_while(|(a, b)| a == b).count();
            ...
            consumed += common;
            self.node_mut(internal_id).terminals.push(Terminal::structural(route, 2, now, expires_at));
            if consumed == tokens.len() {
                return internal_id;
            }
    ...
    pub fn ensure_node(&mut self, tokens: &[i32]) -> usize {
        ...
        let mut consumed = 0;
        loop {
            if consumed == tokens.len() { return node_id; }
            let next = tokens[consumed];
            let child = self.node(node_id).children.iter().copied().find(|child| self.node(*child).fragment[0] == next);
            let Some(child_id) = child else {
                let id = self.push_node(Node { fragment: tokens[consumed..].to_vec(), parent: Some(node_id), children: Vec::new(), terminals: Vec::new() });
                self.node_mut(node_id).children.push(id);
                return id;
            };
            let fragment = &self.node(child_id).fragment;
    ...
    pub fn path(&self, prompt: &[i32], route: SnapshotRoute) -> Vec<(usize, usize)> {
        ...
        let mut consumed = 0;
        loop {
            if self.terminal(node_id, route).is_some() { output.push((node_id, consumed)); }
            if consumed == prompt.len() { break; }
            let Some(child) = self.node(node_id).children.iter().copied().find(|child| self.node(*child).fragment.first() == prompt.get(consumed)) else { break; };
            let fragment = &self.node(child).fragment;
            if prompt.get(consumed..consumed + fragment.len()) != Some(fragment.as_slice()) { break; }
    ...
    pub fn terminal(&self, node: usize, route: SnapshotRoute) -> Option<&Terminal> {
        self.node(node).terminals.iter().find(|terminal| terminal.route == route)
    }

    pub fn terminal_mut(&mut self, node: usize, route: SnapshotRoute) -> Option<&mut Terminal> {
        self.node_mut(node).terminals.iter_mut().find(|terminal| terminal.route == route)
    }

    pub fn remove_terminal(&mut self, node_id: usize, route: SnapshotRoute) -> Option<Terminal> {
        let index = self.node(node_id).terminals.iter().position(|terminal| terminal.route == route)?;
    ...
    pub fn terminal_ids(&self) -> Vec<(usize, SnapshotRoute)> {
        self.nodes.iter().enumerate().filter_map(|(id, node)| node.as_ref().map(|node| (id, node))).flat_map(|(id, node)| node.terminals.iter().map(move |terminal| (id, terminal.route))).collect()
    }
    ...
    fn split_child(&mut self, parent_id: usize, child_id: usize, common: usize) -> usize {
        ...
        let internal = self.push_node(Node { fragment: prefix, parent: Some(parent_id), children: vec![child_id], terminals: Vec::new() });
        self.node_mut(child_id).parent = Some(internal);
        let position = self.node(parent_id).children.iter().position(|id| *id == child_id).expect("child belongs to parent");
    ...
    fn prune(&mut self, mut node_id: usize) {
        while node_id != 0 {
            let (parent, empty, only_child) = {
                ...
                (node.parent.expect("non-root node has parent"), node.terminals.is_empty() && node.children.is_empty(), (node.terminals.is_empty() && node.children.len() == 1).then(|| node.children[0]))
            };
            if empty {
                self.node_mut(parent).children.retain(|child| *child != node_id);
                ...
                let position = self.node(parent).children.iter().position(|id| *id == node_id).expect("node belongs to parent");
    ...
    fn node(&self, id: usize) -> &Node { self.nodes[id].as_ref().expect("live trie node") }
    fn node_mut(&mut self, id: usize) -> &mut Node { self.nodes[id].as_mut().expect("live trie node") }
}
