...
fn divergent_long_prompt_keeps_only_full_checkpoint() {
    ...
    assert!(
        cache
            .lookup(PromptKey::text(&[]), SnapshotRoute::Baseline)
            .is_none()
    );
...
fn half_life_refreshes_are_coalesced_per_entry() {
    ...
    assert!(
        cache
            .lookup(PromptKey::text(&[1, 3]), SnapshotRoute::Baseline)
            .is_some()
    );
    cache.flush_persistence();
    ...
    assert_eq!(
        ...
        vec![(
            entry_key(NAMESPACE, SnapshotRoute::Baseline, PromptKey::text(&[1])),
            300
        )],
    );
}
...
fn in_flight_persistence_is_reserved_against_the_grace_ceiling() {
    ...
    let second_node = cache
        .trie
        .path(PromptKey::text(&[2]), SnapshotRoute::Baseline)
        .into_iter()
        .last()
        .unwrap()
        .0;
...
fn filesystem_hit_survives_hot_eviction_preference() {
    ...
    assert!(
        cache
            .lookup(PromptKey::text(&[3, 4]), SnapshotRoute::Baseline)
            .is_some()
    );
    let hit = cache
        .lookup(PromptKey::text(&[1, 2, 5]), SnapshotRoute::Baseline)
...
fn longest_hit_does_not_refresh_unused_ancestors_and_upserts_are_recent() {
    ...
    assert!(
        cache
            .lookup(PromptKey::text(&[9]), SnapshotRoute::Baseline)
            .is_none()
    );
    let hit = cache
        .lookup(PromptKey::text(&[1, 2]), SnapshotRoute::Baseline)
...
fn memory_only_entry_identity_survives_eviction_and_expiry() {
    ...
    assert!(
        cache
            .lookup(PromptKey::text(&[42]), SnapshotRoute::Baseline)
            .is_none()
    );
    cache.insert(PromptKey::text(&[1]), vec![old], SnapshotRoute::Baseline);
    drop(
...
fn oversized_filesystem_prefix_remains_usable_without_hot_residency() {
    ...
    let expected = state.to_portable().expect("portable snapshot");
    {
        ...
        let hit = cache
            .lookup(PromptKey::text(&[4, 5, 6, 7]), SnapshotRoute::Baseline)
    ...
    let hit = restarted
        .lookup(PromptKey::text(&[4, 5, 6, 8]), SnapshotRoute::Baseline)
...
fn dflash2_rejects_invalid_boundaries_shapes_and_tensor_roles() {
    ...
    for route in [SnapshotRoute::Baseline, SnapshotRoute::Mtp] {
        ...
        assert_ne!(
            entry_key(DFLASH_NAMESPACE, route, PromptKey::text(&[1, 2, 3])),
            encoded.key
        );
    }
}
...
fn dflash2_filesystem_restart_isolates_routes_and_deletes_corrupt_payload() {
    ...
    );
    {
        ...
        assert!(
            cache
                .lookup(PromptKey::text(&[1, 2, 3]), SnapshotRoute::Mtp)
                .is_none()
        );
        cache.flush_persistence();
    ...
    }
    {
        let mut restarted =
            AdaptivePrefixCache::new(namespaces(), config.clone()).expect("restart");
        let hit = restarted
            .lookup(PromptKey::text(&[1, 2, 3, 4]), SnapshotRoute::Dflash2)
...
fn hot_accounting_refreshes_host_mirrors_initialized_after_insertion() {
    ...
    let pinned = cache
        .lookup(PromptKey::text(&tokens[..512]), SnapshotRoute::Baseline)
        .unwrap();
...
fn image_hot_prefixes_isolate_pixels_spans_order_and_text() {
    ...
    cache.insert(
        key,
        vec![snapshot(1, &[1.]), snapshot(3, &[3.]), snapshot(6, &[6.])],
        SnapshotRoute::Baseline,
    );
    ...
    assert_eq!(
        cache
            .lookup(PromptKey::new(&tokens, &changed), SnapshotRoute::Baseline)
            .unwrap()
            .token_count,
        1
    );
    ...
    assert_eq!(
        cache
            .lookup(PromptKey::new(&tokens, &moved), SnapshotRoute::Baseline)
            .unwrap()
            .token_count,
        1
    );
    ...
    assert_eq!(
        cache
            .lookup(PromptKey::new(&tokens, &swapped), SnapshotRoute::Baseline)
            .unwrap()
            .token_count,
        1
    );
    ...
    assert_eq!(
        cache
            .lookup(
                PromptKey::new(&tokens, &changed_later),
                SnapshotRoute::Baseline
            )
            .unwrap()
            .token_count,
    ...
    assert_eq!(
        cache
            .lookup(key, SnapshotRoute::Baseline)
            .unwrap()
            .snapshot()
            .to_portable()
            .unwrap(),
    ...
    assert_eq!(
        cache
            ...
            .to_portable()
            .unwrap(),
        snapshot(6, &[99.]).to_portable().unwrap()
    );
}
...
fn image_dflash_disk_reload_prefetch_and_later_image_extension() {
    ...
    let portable = dflash_portable(3, 1);
    {
        ...
        assert_eq!(
            cache
                .lookup(key, SnapshotRoute::Dflash2)
                .unwrap()
                .snapshot()
                .to_portable()
                .unwrap(),
    ...
    assert!(
        cache
            .lookup(PromptKey::new(&tokens, &changed), SnapshotRoute::Dflash2)
            .is_none()
    );
    assert!(
        cache
            .lookup(PromptKey::text(&tokens), SnapshotRoute::Dflash2)
            .is_none()
    );
    let moved = [image_identity(0, 2, b"red")];
    assert!(
        cache
            .lookup(PromptKey::new(&tokens, &moved), SnapshotRoute::Dflash2)
            .is_none()
    );
