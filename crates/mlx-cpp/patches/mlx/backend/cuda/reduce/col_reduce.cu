...
//   - Kernels write cast_to<V>(accumulated) to output, enabling fp32
//     accumulation with bf16 output for Sum/Prod reductions
//   - Dispatch code passes T (input type) as output type for final writes,
//     U (accumulation type) for two-pass intermediates
//   - Two-pass reduction uses fp32 intermediate for bf16 precision
...
__global__ void col_reduce_looped(
  ...
  // Write result, casting from accumulation type U to output type V.
  if (warp.thread_rank() == 0) {
    if (BLOCKS > 1) {
      out += tile_out * out_size * args.reduction_stride;
    }
    V out_vals[n_outputs];
    for (int i = 0; i < n_outputs; i++) {
      out_vals[i] = cast_to<V>(totals[i]);
    }
...
template <typename T, typename U, typename V, typename Op, int N_READS = 4>
__global__ void col_reduce_small(
  ...
  // Cast from accumulation type U to output type V before storing
  AlignedVector<V, N_READS> out_vals;
  for (int i = 0; i < N_READS; i++) {
    out_vals[i] = cast_to<V>(accumulator[i]);
  }
  store_vector(out, 0, out_vals);
}
  ...
  dispatch_all_types(in.dtype(), [&](auto type_tag) {
    dispatch_reduce_ops(reduce_type, [&](auto reduce_type_tag) {
      dispatch_reduce_ndim(args.reduce_ndim, [&](auto reduce_ndim) {
        ...
        // Output type is T (matches input dtype = out.dtype())
        auto kernel =
            cu::col_reduce_looped<T, U, T, OP, reduce_ndim(), BM, BN, N_READS>;
...
void col_reduce_small(
  ...
  dispatch_all_types(in.dtype(), [&](auto type_tag) {
    dispatch_reduce_ops(reduce_type, [&](auto reduce_type_tag) {
      ...
      // Output type is T (matches input dtype = out.dtype())
      auto kernel = cu::col_reduce_small<T, U, T, OP, N_READS>;
      encoder.add_kernel_node(
          kernel,
          grid,
          block,
...
void col_reduce_two_pass(
  ...
  // Determine intermediate dtype: use accumulation type for precision.
  // For bf16 Sum/Prod this gives float32; for other types it matches out.dtype()
  ...
  array intermediate(intermediate_shape, intermediate_dtype, nullptr, {});
  ...
  // Single dispatch on the original input type handles both passes.
  // This avoids N^2 template instantiations from nested dispatch_all_types.
  ...
  dispatch_all_types(in.dtype(), [&](auto type_tag) {
    dispatch_reduce_ops(reduce_type, [&](auto reduce_type_tag) {
      dispatch_reduce_ndim(args.reduce_ndim, [&](auto reduce_ndim) {
        ...
        // --- First pass: T input -> U intermediate ---
        {
          // Cub doesn't like const pointers for vectorized loads. (sigh)
          T* indata = const_cast<T*>(gpu_ptr<T>(in));
          ...
          auto kernel = cu::col_reduce_looped<
              T,
              U,
              U,
              OP,
        ...
        // --- Second pass: U intermediate -> T output ---
        {
          ...
          dispatch_reduce_ndim(
              second_args.reduce_ndim, [&](auto reduce_ndim2) {
                dim3 grid =
                    output_grid_for_col_reduce(out, second_args, BN);
                int blocks = BM * BN / N_READS;

                // Second pass: U input, U accumulation, T output
                auto kernel = cu::col_reduce_looped<
                    U,
                    U,
                    T,
