...
//   - Kernels write static_cast<V>(accumulated) to output, enabling fp32
//     accumulation with bf16 output for Sum/Prod reductions
//   - Dispatch code passes T (input type) as output type
...
template <
    ...
    int M = 1>
__global__ void
row_reduce_simple(const T* in, V* out, size_t n_rows, int size) {
  ...
  if (block.thread_rank() == 0) {
    if (grid.block_rank() * M + M <= n_rows) {
      // Cast accumulator values to output type before storing
      AlignedVector<V, M> out_vals;
      for (int i = 0; i < M; i++) {
        out_vals[i] = cast_to<V>(accs[i]);
      }
      store_vector(out, 0, out_vals);
    } else {
      short offset = grid.block_rank() * M + M - n_rows;
      for (int i = offset; i < M; i++) {
        out[i] = cast_to<V>(accs[i]);
      }
    }
  }
}
...
void row_reduce_simple(
  ...
  dispatch_all_types(in.dtype(), [&](auto type_tag) {
    dispatch_reduce_ops(reduce_type, [&](auto reduce_type_tag) {
      ...
      // Pick the kernel: output type is T (input type = output dtype)
      auto kernel = cu::row_reduce_simple<T, U, T, OP, N_READS>;
      if (grid.x >= 1024) {
        grid.x = (grid.x + 1) / 2;
        kernel = cu::row_reduce_simple<T, U, T, OP, N_READS, 2>;
      }

      T* indata = const_cast<T*>(gpu_ptr<T>(in));
      int size = plan.shape.back();
      encoder.add_kernel_node(
          kernel, grid, block, indata, gpu_ptr<T>(out), out.size(), size);
    });
  });
}
...
void row_reduce_looped(
  ...
  dispatch_all_types(in.dtype(), [&](auto type_tag) {
    dispatch_reduce_ops(reduce_type, [&](auto reduce_type_tag) {
      ...
      // Pick the kernel: output type is T (input type = output dtype)
      auto kernel = cu::row_reduce_looped<T, U, T, OP, 1, N_READS>;
      dispatch_reduce_ndim(args.reduce_ndim, [&](auto reduce_ndim) {
        kernel =
            cu::row_reduce_looped<T, U, T, OP, reduce_ndim.value, N_READS>;
      });

      encoder.add_kernel_node(
          kernel, grid, block, gpu_ptr<T>(in), gpu_ptr<T>(out), args);
    });
  });
}
