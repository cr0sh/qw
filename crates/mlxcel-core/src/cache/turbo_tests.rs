...
fn turbo4_causal_attention_preserves_per_position_shape_and_offset() {
    ...
    assert!(cache.v_packed.is_some());
    if crate::metal_is_available() && super::turbo::fused_attention::turbo4_grouped_causal_enabled()
    {
...
fn turbo4_grouped_causal_rows_3_to_5_match_rowwise_packed_attention() {
    ...
    // Exercise long ragged stages and the 65,536-token reduction-tier crossing.
    for prefix_len in [64_585, 65_534] {
        for (query_len, seed) in [(3_i32, 0xA301_u32), (4, 0xA401), (5, 0xA501)] {
            ...
            for row in 0..query_len {
                ...
                rows.push(
                    rowwise
                        .turbo4_fused_attention_prefix(&q_row, prefix_len + row + 1, scale, true)
            ...
            assert_eq!(
                ...
                "prefix={prefix_len} grouped qlen={query_len} output changed from rowwise packed attention"
            );
        }
    }
}
