...
fn load_boundary_rejects_invalid_qtypes_shapes_lengths_and_overflow() {
    assert!(matches!(
        GgmlQuantizedMatrix::from_bytes(&[], 2, 256, 1),
        Err(GgmlQuantError::UnsupportedQType(2))
    ));
    assert!(matches!(
        GgmlQuantizedMatrix::from_bytes(&[], 11, 255, 1),
        Err(GgmlQuantError::UnalignedWidth { .. })
    ));
    assert!(matches!(
        GgmlQuantizedMatrix::from_bytes(&[0; 109], 11, 256, 1),
    ...
    assert!(matches!(
        GgmlQuantizedMatrix::from_bytes(&[], 0, usize::MAX, 2),
...
fn dispatch_stats_record_zero_workspace_and_shape_selected_traffic() {
    ...
    let prefill = matrix.dispatch_stats(5).unwrap();
    assert_eq!(prefill.path, GgmlKernelPath::PrefillRows4);
    assert_eq!(prefill.packed_bytes_read, packed.len() * 2);
...
fn metal_matrix_and_embedding_match_reference_for_every_target_qtype() {
    ...
    for qtype in GgmlQType::TARGET_TYPES {
        ...
        let prefill = matrix.forward(input.as_ref().unwrap()).unwrap();
        let prefill_values = raw_f32(prefill.as_ref().unwrap());
        for output in 0..output_rows {
            assert!(
                ulp_distance(decode_values[output], prefill_values[output]) <= 1,
                "{qtype:?} M=1/prefill mismatch: {} vs {}",
                decode_values[output],
                prefill_values[output]
            );
        }
        for input_row in 0..5 {
            for output in 0..output_rows {
                let expected = prefill_input[input_row * width..(input_row + 1) * width]
                    .iter()
                    .zip(&weights[output])
                    .map(|(input, weight)| input * weight)
                    .sum::<f32>();
                let actual = prefill_values[input_row * output_rows + output];
                let tolerance = 0.002 + expected.abs() * 0.00002;
                assert!(
                    (actual - expected).abs() <= tolerance,
                    "{qtype:?} prefill input {input_row} output {output}: actual {actual}, expected {expected}"
                );
            }
        }
        let embedding =
            GgmlQuantizedEmbedding::from_bytes(&packed, qtype.id(), width, output_rows).unwrap();
    ...
    for (model_key, path_variable, qtypes) in [
        ...
        for &qtype in qtypes {
            ...
            let activation: Vec<f32> = (0..width)
                ...
                .collect();
            let input = crate::from_slice_f32(&activation, &[1, width as i32]);
            let matrix = GgmlQuantizedMatrix::from_bytes(&packed, qtype.id(), width, 1).unwrap();
